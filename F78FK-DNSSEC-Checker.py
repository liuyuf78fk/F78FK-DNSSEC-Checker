...
def determine_result(nic, failed):
    ...
    logs.append(f"Parsed dnssec-failed.org: AD={failed['ad']}, Status={failed['status']}")
    ...
    if nic["ad"] and not failed["ad"] and failed["status"] == "SERVFAIL":
    ...
    elif not nic["ad"] and not failed["ad"] and failed["status"] == "SERVFAIL":
        result = "medium"
        ...
        result = "unknown"
        logs.append("DNSSEC status indeterminate, returning 'unknown'")
...
async def handler(websocket):
    ...
    failed_result = query_domain("dnssec-failed.org", ["+dnssec", "dnssec-failed.org", "A"])
...
if __name__ == "__main__":
    try:
        ...
        print("[WARNING] Current environment does not support asyncio.run(), please run in local terminal.")
